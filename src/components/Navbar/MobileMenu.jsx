      ...
      <div className="flex">
        ...
        <motion.div style={{ transitionDuration: '.2s', clipPath: clipstate }} initial='initial' animate={`${isOpen ? 'animate' : ''}`} variants={clipanim} className="w-full absolute h-[calc(100vh)] top-[0px] left-[0px] pt-[7rem] px-[1.5rem] sm:px-[3rem] pb-[calc(100lvh-100svh+5rem)] bg-white overflow-auto z-[-1] ">
          <div className="flex flex-col gap-y-4 overflow-auto">
            <div className={`flex flex-col justify-between rounded-[35px] bg-[#E6E8FE] text-xl`}>
              <div className="flex flex-row py-[1.25rem] px-[1.5rem] items-center justify-between" onClick={() => toggleDropdown("why-realtern")}>
              ...
              {openDropdown === "why-realtern" && (
                <WhyRealternMobileMenu openDropdown={true} />
              )}
            </div>
            <div className="flex flex-col gap-y-4 overflow-auto">
              <div className={`flex flex-col justify-between rounded-[35px] bg-[#E6E8FE] text-xl`}>
                <div className="flex flex-row py-[1.25rem] px-[1.5rem] items-center justify-between" onClick={() => toggleDropdown("community")}>
                ...
                {openDropdown === "community" && (
                  <Individualmobile openDropdown={true} />
                )}
              </div>
            </div>
            <div className="flex flex-col gap-y-4 overflow-auto">
              <div className={`flex flex-col justify-between rounded-[35px] bg-[#E6E8FE] text-xl`}>
                <div className="flex flex-row py-[1.25rem] px-[1.5rem] items-center justify-between" onClick={() => toggleDropdown("startup-glimpse")}>
                ...
                {openDropdown === "startup-glimpse" && (
                  <Aboutusmobile openDropdown={true} />
                )}
              </div>
            </div>
          </div>
